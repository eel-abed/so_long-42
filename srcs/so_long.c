...
/*   Updated: 2024/04/30 18:52:24 by eel-abed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_randomize(void* param)
{
	...
	for (uint32_t i = 0; i < image->width; ++i)
	{
		...
		{
			uint32_t color = ft_pixel(255, 255, 255, 255); // White color
...
{
	mlx_t* mlx = param;
	int32_t move_amount = 5;

	if (mlx_is_key_down(mlx, MLX_KEY_ESCAPE))
		mlx_close_window(mlx);
	if (mlx_is_key_down(mlx, MLX_KEY_UP) && image->instances[0].y - move_amount >= 0)
		image->instances[0].y -= move_amount;
	if (mlx_is_key_down(mlx, MLX_KEY_DOWN) && image->instances[0].y + move_amount <= (int)HEIGHT - (int)image->height)
		image->instances[0].y += move_amount;
	if (mlx_is_key_down(mlx, MLX_KEY_LEFT) && image->instances[0].x - move_amount >= 0)
		image->instances[0].x -= move_amount;
	if (mlx_is_key_down(mlx, MLX_KEY_RIGHT) && image->instances[0].x + move_amount <= (int)WIDTH - (int)image->width)
		image->instances[0].x += move_amount;
}
// -----------------------------------------------------------------------------
